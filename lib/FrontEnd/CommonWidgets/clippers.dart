...
class CustomShapeClipper extends CustomClipper<Path> {
  ...
  Path getClip(Size size) {
    ...
    var secondControlPoint = Offset(size.width * .83, size.height - 160);
...
class CustomShapeClipperHorizontalCropping extends CustomClipper<Path> {
  @override
  Path getClip(Size size) {
    final Path path = Path();
    ...
    var firstControlpoint = Offset(size.width / 2, size.height / 2.5);
    path.quadraticBezierTo(firstControlpoint.dx, firstControlpoint.dy,
        firstEndPoint.dx, firstEndPoint.dy);
...
class CustomShapeCopyClipper extends CustomClipper<Path> {
  ...
  Path getClip(Size size) {
    ...
    var secondControlPoint = Offset(size.width * .80, size.height - 120);
    path.quadraticBezierTo(secondControlPoint.dx, secondControlPoint.dy,
        secondEndPoint.dx, secondEndPoint.dy);
    path.lineTo(size.width, 0.0);
    path.close();
